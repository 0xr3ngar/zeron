...
export class VaultRoom implements DurableObject {
  ...
  constructor(ctx: DurableObjectState, _env: Env) {
    ...
    ctx.storage.sql.exec(
      `CREATE TABLE IF NOT EXISTS meta (key TEXT PRIMARY KEY, value TEXT NOT NULL);
       ...
       CREATE TABLE IF NOT EXISTS credentials (id INTEGER PRIMARY KEY CHECK(id = 1), revision INTEGER NOT NULL, record BLOB NOT NULL);
  ...
  async fetch(request: Request): Promise<Response> {
    ...
    if (parts.length === 1 && parts[0] === "credentials") {
      if (method === "GET") return json(this.credentials());
      if (method === "PUT") return this.putCredentials(request);
  ...
  private credentials(): { revision: number; record: string } {
    const row = [...this.sql.exec<{ revision: number; record: ArrayBuffer }>("SELECT revision, record FROM credentials WHERE id = 1")][0];
    return row ? { revision: row.revision, record: encodeBase64(new Uint8Array(row.record)) } : { revision: 0, record: "" };
  }

  private async putCredentials(request: Request): Promise<Response> {
    const revision = Number(request.headers.get("x-zeron-revision"));
    if (!Number.isSafeInteger(revision) || revision < 1) return json({ error: "invalid_revision" }, 400);
    const body = await readBody(request, 1024 * 1024);
    if (!body) return json({ error: "too_large" }, 413);
    if (!looksLikeSealedContent(body, 1024 * 1024, 10n)) return json({ error: "sealed_credentials_required" }, 400);
    const head = this.head();
    if (!head) return json({ error: "not_found" }, 404);
    const record = parseSignedRecord(body, 1024 * 1024);
    if (new DataView(record.revisionId.buffer, record.revisionId.byteOffset, 16).getBigUint64(0) !== BigInt(revision)) return json({ error: "unsigned_revision" }, 400);
    const b = record.binding;
    // Rust object_id_for("credentials", "accounts"); covered by the live cross-language test.
    if (!bytesEqual(b.vaultId, head.vaultId) || !bytesEqual(b.generation, head.generation) ||
        !bytesEqual(b.objectId, unhex("c93f41aeb1dd5c9828e85a98637f3457", 16)!)) return json({ error: "wrong_vault" }, 400);
    if (b.epoch !== head.epoch || !bytesEqual(b.membershipHash, head.hash)) return json({ error: "stale_parent" }, 409);
    const author = head.payload.devices.find(d => bytesEqual(d.deviceId, b.authorId) && d.status === DeviceStatus.active);
    if (!author || !(await verifyEd25519(author.signingKey, record.signingInput, record.signature))) return json({ error: "invalid_author" }, 403);
    // Recheck after WebCrypto yields. Retries resend the exact sealed bytes;
    // an acknowledged-but-lost response must not duplicate the mutation.
    return this.ctx.storage.transactionSync(() => {
      if (!this.headMatches(head.hash)) return json({ error: "stale_parent" }, 409);
      const current = this.credentials();
      if (revision === current.revision && current.record === encodeBase64(body)) return json(current);
      if (revision !== current.revision + 1) return json({ error: "conflict" }, 409);
      this.sql.exec("INSERT INTO credentials (id, revision, record) VALUES (1, ?, ?) ON CONFLICT(id) DO UPDATE SET revision = excluded.revision, record = excluded.record", revision, buffer(body));
      return json({ revision, record: encodeBase64(body) });
    });
  }
