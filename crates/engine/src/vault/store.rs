...
pub struct LocalVaultState {
    pub version: u32,
    #[serde(default)]
    pub credential_pin: Option<(u64, String)>,
    #[serde(default)]
    pub pending_credentials: Option<(u64, String)>,
    #[serde(default)]
...
impl VaultStore {
    ...
    fn aad(&self) -> Vec<u8> {
        [FILE_AAD, self.profile_key.as_bytes()].concat()
    }
    ...
    pub fn save(&self, state: &LocalVaultState) -> Result<(), VaultStoreError> {
        ...
        let ciphertext =
            zeron_crypto::seal_aes256_gcm(key.as_bytes(), &nonce, &self.aad(), &plaintext)?;
        let file = StateFile {
            version: FILE_VERSION,
...
/// Use native OS credential APIs: never pass key material in process arguments.
/// Missing/locked key stores fail closed; an existing unreadable item is never
/// replaced. Profile runtimes hold the engine's exclusive profile instance lock.
pub struct NativeProtection;
impl ProtectionKeyProvider for NativeProtection {
    fn mode(&self) -> ProtectionMode {
        ProtectionMode::Keychain
    }
    fn load_or_create(&self, profile_key: &str) -> Result<SecretBytes, VaultStoreError> {
        let entry = keyring::Entry::new("sh.zeron.vault", profile_key)
            .map_err(|_| VaultStoreError::Locked("OS credential store is unavailable".into()))?;
        match entry.get_password() {
            Ok(value) => {
                let encoded = Secret(Hex(value));
                let decoded = zeroize::Zeroizing::new(encoded.bytes().unwrap_or_default());
                if decoded.len() != 32 {
                    return Err(VaultStoreError::Locked(
                        "invalid stored protection key".into(),
                    ));
                }
                Ok(SecretBytes::from_slice(&decoded))
            }
            Err(keyring::Error::NoEntry) => {
                let mut key = zeroize::Zeroizing::new([0u8; 32]);
                zeron_crypto::fill_random(key.as_mut())?;
                let encoded = zeroize::Zeroizing::new(Hex::of(key.as_ref()).0);
                entry.set_password(&encoded).map_err(|_| {
                    VaultStoreError::Locked("OS credential store refused the protection key".into())
                })?;
                // Verify persistence before encrypting any durable data.
                let stored = zeroize::Zeroizing::new(entry.get_password().map_err(|_| {
                    VaultStoreError::Locked(
                        "OS credential store could not read back the protection key".into(),
                    )
                })?);
                if *stored != *encoded {
                    return Err(VaultStoreError::Locked(
                        "protection key changed during creation".into(),
                    ));
                }
                Ok(SecretBytes::from_slice(key.as_ref()))
            }
            Err(_) => Err(VaultStoreError::Locked(
                "OS credential store is locked or unavailable".into(),
            )),
        }
    }
}
...
mod tests {
    ...
    #[test]
    fn a_shared_protection_key_cannot_open_another_profiles_state() {
        let dir = tempfile::tempdir().unwrap();
        let key = dir.path().join("protection-key");
        write_private(&key, &[19; 32]).unwrap();
        ...
        let original = open("org/first-user");
        original.save(&LocalVaultState::default()).unwrap();
        assert!(original.load().is_ok());
        assert!(open("org/second-user").load().is_err());
        assert!(open("other-org/first-user").load().is_err());
        assert!(
            original.load().is_ok(),
            "failed opens must leave the original intact"
        );
    }
