...
impl VaultService {
    ...
    /// Local anti-rollback pin and a journaled write. Only authenticated
    /// ciphertext is persisted here; an interrupted PUT is replayed byte-for-byte.
    pub fn credential_state(&self) -> (Option<(u64, String)>, Option<(u64, String)>) {
        let g = lock(&self.inner.guarded);
        (
            g.state.credential_pin.clone(),
            g.state.pending_credentials.clone(),
        )
    }

    pub fn save_credential_state(
        &self,
        pin: Option<(u64, String)>,
        pending: Option<(u64, String)>,
    ) -> Result<(), EngineError> {
        let mut g = lock(&self.inner.guarded);
        if g.content_blocked() || g.trust.is_none() {
            return Err(EngineError::Other("vault is not ready".into()));
        }
        if let (Some((before, _)), Some((after, _))) = (&g.state.credential_pin, &pin) {
            if after < before {
                return Err(EngineError::Other(
                    "credential history moved backwards".into(),
                ));
            }
        }
        let old_pin = g.state.credential_pin.clone();
        let old_pending = g.state.pending_credentials.clone();
        g.state.credential_pin = pin;
        g.state.pending_credentials = pending;
        if let Err(e) = self.commit(&mut g) {
            g.state.credential_pin = old_pin;
            g.state.pending_credentials = old_pending;
            return Err(e);
        }
        Ok(())
    }
