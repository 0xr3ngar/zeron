...
impl VaultClient {
    ...
    async fn json<T: serde::de::DeserializeOwned>(
        ...
        while let Some(chunk) = res
            .chunk()
            .await
            .map_err(|_| EngineError::Other("vault response interrupted".into()))?
        {
            if bytes.len() + chunk.len() > 8 * 1024 * 1024 {
                return Err(EngineError::Other("vault response too large".into()));
            }
            bytes.extend_from_slice(&chunk);
        }
        serde_json::from_slice(&bytes)
            .map_err(|_| EngineError::Other("malformed vault response".into()))
    }

    pub async fn credentials(
        &self,
    ) -> Result<crate::shared_credentials::CredentialEnvelope, EngineError> {
        let response = self
            .send(
                self.http
    ...
    /// False is a CAS conflict. Transport failure is ambiguous: the caller
    /// retains the exact ciphertext in its durable journal and retries it.
    pub async fn put_credentials(
        &self,
        envelope: &crate::shared_credentials::CredentialEnvelope,
    ) -> Result<bool, EngineError> {
        let bytes = decode_base64(&envelope.record)
            .ok_or_else(|| EngineError::Other("invalid credential encoding".into()))?;
        let response = self
            .send(
                self.http
                    .put(self.url("/credentials"))
                    .bearer_auth(self.bearer().await?)
                    .header("content-type", "application/octet-stream")
                    .header("x-zeron-revision", envelope.revision.to_string())
                    .body(bytes),
            )
            .await?;
        ...
        if !response.status().is_success() {
            return Err(EngineError::Other(format!(
                "credential write http {}",
                response.status().as_u16()
            )));
        }
        let acknowledged: crate::shared_credentials::CredentialEnvelope =
            Self::json(response).await?;
        if acknowledged.revision != envelope.revision || acknowledged.record != envelope.record {
            return Err(EngineError::Other(
                "credential acknowledgement does not match".into(),
            ));
        }
        Ok(true)
    }
