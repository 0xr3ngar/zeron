...
/// Sequence is signed in the public revision id, so the edge can enforce
/// CAS without decrypting metadata. Fresh random salt still derives a unique
/// encryption key even when concurrent writers propose the same sequence.
