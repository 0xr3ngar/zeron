...
#[test]
fn credential_sequences_are_signed_and_each_seal_has_fresh_key_material() -> TestResult {
    let (key, signer) = keys()?;
    let a = seal_credentials(&binding(), 7, &key, &signer, b"credential canary", 1024)?;
    let b = seal_credentials(&binding(), 7, &key, &signer, b"credential canary", 1024)?;
    assert_eq!(&a.revision_id()[..8], &7u64.to_be_bytes());
    assert_ne!(a.encoded(), b.encoded());
    assert!(seal_credentials(&binding(), 0, &key, &signer, b"key", 1024).is_err());
    let parsed = UnverifiedRecord::parse(a.encoded(), 2048)?;
    let mut revision = *parsed.untrusted_revision_id();
    revision[..8].copy_from_slice(&8u64.to_be_bytes());
    // Re-encoding the header with the original signature cannot relabel it.
    let signature: &[u8; 64] = a.encoded()[a.encoded().len() - 64..].try_into()?;
    let modified = record::encode_signed(
        &binding(),
        &revision,
